...
- Dependabot no longer proposes a TypeScript version Angular cannot accept. Angular 22 declares
  ...
  were held and a 6.1 release would have reproduced the failure. zone.js is held the same way as a
  precaution: Angular accepts only its 0.15 and 0.16 lines, and because it is a 0.x package a future 0.17
  would arrive classified as a minor, batched into the routine group update.
- The Docker image build no longer fails intermittently on GitHub API rate limiting. It resolves the JDK and
  ...
  CI now passes its job token into the build. The token is optional, so building without one (Cloud Build for
  the production deploy, or a local `docker build`) is unchanged, and it never reaches the published image.
...
- Local dev now publishes the PostgreSQL container on host port `5433` instead of the conventional `5432`,
  overridable with the `DB_PORT` environment variable, so it does not collide with a system Postgres or
  another project already bound to `5432` (the same collision-avoidance as naming the container
  `campus-coffee-db`). The container-internal port and the app-to-database connection inside Docker Compose
  stay `5432`; only the host-published port and the local dev datasource change. Testcontainers-based tests
  are unaffected (they use random host ports).
...
- Deactivating a user who still owes the coffee fund is now refused. An admin cannot flip a user inactive
...
- Activity and ratings UI refinements. On the admin Activity table an expense or a rating shows its detail on
  separate lines (the bean name or note, then the weight or the value out of five) rather than crammed onto
  one, each truncated with a tooltip, and the table's tooltips are native so the browser anchors them at the
  cursor. A bean with no ratings shows just its empty icons on the Ratings page, with no "no votes" label
  (the Votes column already reads "0 votes").
...
[1.1.0]: https://github.com/se-uhd/campus-coffee-consumption/releases/tag/v1.1.0
