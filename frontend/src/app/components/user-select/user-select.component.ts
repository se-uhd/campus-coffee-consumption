...
/**
 * The shared user picker the admin pages use to choose whose data to view (the landing, expenses, and the
 * admin-mode profile). A presentational dropdown over the user list: the trigger and each option show the
 * user's login name and full name, and the admin's own account is marked with a leading person icon (the
...
export class UserSelectComponent {
  /** The users the admin may switch between. */
  readonly users = input<UserDto[]>([]);

  /** The id of the currently-selected user (the bound value of the dropdown). */
  ...
  /** Emits the newly-picked user id when the admin changes the selection. */
  readonly selectionChange = output<string>();

  /** The currently-selected user, resolved from {@link users} by the bound id, for the trigger. */
