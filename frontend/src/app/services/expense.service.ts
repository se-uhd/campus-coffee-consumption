 ...
 * An admin records a purchase for a member with the explicit kitty/private split, and can correct or
...
export class ExpenseService {
  ...
  /** Lists a member's recorded bean purchases (with their ids), so they can be corrected or deleted. */
  adminList(userId: string): Promise<ExpenseDto[]> {
    return firstValueFrom(this.http.get<ExpenseDto[]>(`/api/users/${userId}/expenses`));
  }

  /** Records a bean purchase for a member with the kitty/private split. */
