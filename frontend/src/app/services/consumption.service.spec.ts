...
describe('ConsumptionService', () => {
  ...
  it("POSTs to /cancel to undo a user's most recent coffee for an admin", async () => {
    const expected: ConsumptionDto = { total: 0, changes: [] };
    const promise = service.cancelForUser('user-1');
    const req = httpMock.expectOne('/api/users/user-1/consumption/cancel');
    expect(req.request.method).toBe('POST');
    req.flush(expected);
    expect(await promise).toEqual(expected);
  });
