...
- **CORS:** none is configured, and the plan **assumes none is needed**, because the SPA and the API are
  ...
  in prod the browser loads the app and calls `/api/...` under one scheme+host+port, so the browser never
    ...
    spring:
      config:
        activate:
          on-profile: prod
      datasource:
        url: "jdbc:postgresql:///postgres?cloudSqlInstance=${CLOUD_SQL_INSTANCE}&socketFactory=com.google.cloud.sql.postgres.SocketFactory"
...
capability-token-vs-JWT auth split, and Cloud SQL deploy. Start a **fresh `CHANGELOG.md` at version `0.1.0`** (this is a new project, not a continuation of
