...
gradle=$(sed -n 's/^version *= *\([0-9][0-9]*\.[0-9][0-9]*\.[0-9][0-9]*\).*/\1/p' gradle.properties)
[ -n "$gradle" ] || fail "could not read the version from gradle.properties"

# The latest released version header in CHANGELOG.md (## [x.y.z]) must match it. The [Unreleased] header
# has no digits, so it is skipped, and the first match is the newest release.
