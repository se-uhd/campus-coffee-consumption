 ...
 * entities. Modeled on CampusCoffee's review entity mapper. On the write side ([toEntity]/[updateEntity]) the
 * [CoffeeConsumption.user] association is set as a managed reference by id via the [EntityReferenceResolver],
 * so a write only sets the foreign key and never rewrites the referenced user row; on the read side
 * [fromEntity] maps it deeply through the [UserEntityMapper]. The entity's `version` column (data-layer
