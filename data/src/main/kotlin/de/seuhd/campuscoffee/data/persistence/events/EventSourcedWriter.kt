 ...
 *
 * After projecting an event it also refreshes the [BalanceProjectionMaintainer] in the same transaction, so the
...
class EventSourcedWriter(
    ...
    fun <D : DomainModel<UUID>> upsert(
        ...
        if (existingId == null) {
            ...
            event = eventAppender.appendInsert(complete)
        } else {
            complete = buildForUpdate(getById(existingId), now)
            event = eventAppender.appendUpdate(complete)
        }
        project(event)
    ...
    fun <D : Any> delete(
        ...
        project(eventAppender.appendDelete(domainType, id, ownerKeys(loaded)))
    }
    ...
    fun clear(
        ...
        eventAppender.clear(eventAppender.entityTypeOf(domainType))
        clearReadModel()
