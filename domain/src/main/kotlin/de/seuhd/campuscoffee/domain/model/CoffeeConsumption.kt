...
/**
 * Immutable coffee-consumption domain model: a single user's running coffee count. It is modeled
 ...
 *
 * Every change to [count] (a user adding a coffee, a user undoing a recent one, or an admin override
 * correcting the count) is a plain upsert that the event-sourced data adapter records as a full-state event
 * in the append-only log; the per-user transaction history is read back from that log.
 *
