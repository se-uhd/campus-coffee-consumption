...
/**
 * One row of the admin-only **global** activity feed: every change across all members, the kitty, and the
 * price, reconstructed from a single event-log row (there is no activity table). It is a read-only projection,
 * the all-members analogue of [ActivityEntry] but carrying two running balances and two users, because one
 * event can move both a member's balance and the kitty (a deposit, a split bean purchase).
 *
 * Each row records who it concerns ([subjectUserId]/[subjectLogin]/[subjectName], the member whose balance or
 * count moved) and who performed it ([actorLogin], the event's `created_by`). For a member self-scan the two
 * coincide; for an admin correction they differ. The subject is null for an event that concerns no single
 * member (a pure kitty adjustment or a price change); for a hard-deleted member whose events outlive them, the
 * subject id remains but the login does not resolve, so [subjectLogin] is null and [subjectName] reads
 * `(deleted member)`.
 *
 * [memberEffectCents]/[memberBalanceCents] are this event's signed effect on the subject member's balance and
 * that balance afterwards, both null when the event moves no member balance (a kitty adjustment, a price
...
data class GlobalActivityEntry(
    ...
    val note: String?,
    val memberEffectCents: Long?,
    val memberBalanceCents: Long?,
