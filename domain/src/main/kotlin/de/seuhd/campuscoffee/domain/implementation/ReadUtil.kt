 ...
 * requested page, clamping the limit to [MAX_ACTIVITY_PAGE]. Shared by the user, kitty, summary, and global
...
/**
 * Strips both portions of a split expense from an activity entry (the user-serving views): the split is
 * admin-only and must never reach a user. A no-op for an entry that carries none.
 */
...
/**
 * Resolves the user (404 if missing) and allows the read only for the user themselves or an admin.
 *
 * @param userId the user whose data is being read
 * @param actingUser the authenticated user attempting the read
 * @param userDataService the port used to resolve the user
 */
internal fun requireMayViewUser(
    ...
    if (actingUser.role != Role.ADMIN && actingUser.persistedId != userId) {
        throw ForbiddenException("A user's balance may be read only by the user themselves or an admin.")
    }
    return user
}
