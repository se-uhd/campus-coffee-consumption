 ...
 *
 * Every change to [count] (a relative `+`/`-` adjustment, an admin override, or a reset to zero after
 * payment) is a plain upsert that the event-sourced data adapter records as a full-state event in the
 * append-only log; the per-member transaction history is read back from that log.
 *
