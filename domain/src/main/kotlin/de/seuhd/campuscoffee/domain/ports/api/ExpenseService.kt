 ...
 *
 * A member records only their own purchases, always paid 100% from their own pocket (the split and the
 * buyer are server-derived, never taken from the request). Only an admin may record a purchase with a
 * kitty/private split or attributed to another member, and only an admin may correct or delete a purchase.
 * The two split portions must always sum to the total.
 */
interface ExpenseService {
    /**
     * Records a purchase the calling member made with their own money (100% private to themselves).
     *
     ...
     * @param actingUser  the member recording their own purchase
    ...
    /**
     * Records a purchase on behalf of a member, with an explicit kitty/private split (admin-only).
     *
     * @param buyerUserId        the member credited with the private portion
     * @param weightGrams        the bean weight in grams (non-negative)
    ...
    /**
     * Lists a member's recorded bean purchases (admin-only), so an admin can find one to correct or delete.
     *
