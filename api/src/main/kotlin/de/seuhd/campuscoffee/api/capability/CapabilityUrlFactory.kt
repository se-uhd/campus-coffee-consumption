 ...
 * entry point (`/coffee/{token}`); API calls forward it as the `X-Coffee-Token` header instead, keeping it
...
class CapabilityUrlFactory(
    ...
    /**
     * Assembles the capability URL for the given token: `"<base-url>/coffee/<token>"`.
     *
    ...
    fun urlFor(capabilityToken: String): String =
        "${appProperties.baseUrl.trimEnd('/')}$COFFEE_ROUTE_PREFIX$capabilityToken"

    private companion object {
        private const val COFFEE_ROUTE_PREFIX = "/coffee/"
    }
}
