 ...
 * given member's activity feed (their unified ledger). Kept separate from [UserController] for cohesion;
 * paging is validated through the shared [PageQuery] object (see its KDoc for why the project validates
 * paging via `@Valid` binding rather than a class-level `@Validated`).
 */
    ...
    fun memberActivity(
        ...
        return ResponseEntity.ok(
            ...
            accountingDtoMapper.toEntryDtos(
                accountingService.memberLedger(
                    userId,
