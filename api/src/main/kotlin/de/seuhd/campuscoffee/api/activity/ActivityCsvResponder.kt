package de.seuhd.campuscoffee.api.activity
 ...
 * first so a spreadsheet (notably Excel) detects the encoding and renders member names with umlauts correctly,
...
class ActivityCsvResponder {
    ...
    private fun row(entry: GlobalActivityEntry): List<Any?> =
        listOf(
            ...
            entry.type.name,
            // the free-text cells are user-controlled (a member edits their own name and writes notes), so guard
            ...
            entry.memberEffectCents,
            entry.memberBalanceCents,
