...
/**
 * Authenticates a user by their secret capability token. Reads the `X-Capability-Token` header, resolves it
 * to a [de.seuhd.campuscoffee.domain.model.User] via [UserService.findByCapabilityToken], and on a
 * match sets a `ROLE_USER` principal (the user's login name) on the security context. The capability
 ...
 * A deactivated user is still authenticated here (so their reads work); the domain rejects their
...
class CapabilityTokenAuthenticationFilter(
    ...
    override fun doFilterInternal(
        ...
        if (!token.isNullOrBlank() && SecurityContextHolder.getContext().authentication == null) {
            userService.findByCapabilityToken(token)?.let { user ->
                val authentication =
