...
/**
 * Self-service profile controller for the authenticated user (the capability token principal): it views
...
class ProfileController(
    ...
    /**
     * Updates the authenticated user's own profile: first name, last name, and email only. The update
     * is pinned to the caller's own id and login name, and the server-owned fields a user must not change
     * through their profile (the password, role, and active flag) are dropped, so a profile edit can change
     * nothing beyond the three intended fields (a user authenticates only with their capability token and
     * never sets a password or renames their login here).
     *
     * @param dto the profile fields to update
     */
    @Operation(summary = "Update the authenticated user's own profile (first name, last name, email).")
    @PutMapping("")
    fun update(
    ...
    @Operation(summary = "Download the user's own capability QR code (high-resolution PNG).")
