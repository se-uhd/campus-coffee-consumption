...
class ProfileController(
    ...
    fun update(
        ...
        // a member may edit only first name, last name, and email: pin id and login name to the caller's
        // own values and drop the server-owned fields (password/role/active)
        val toUpdate =
            userDtoMapper
                .toDomain(dto)
                .copy(
                    id = current.persistedId,
