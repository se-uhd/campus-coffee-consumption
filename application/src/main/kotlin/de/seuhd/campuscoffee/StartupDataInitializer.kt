...
/**
 * Runs every registered [StartupTaskService] before the embedded web server starts accepting requests, so the API
 ...
 *
 * Spring injects every [StartupTaskService] bean present (each is conditional on its own property), and they run in
 * ascending [StartupTaskService.order]: the optional event-log rebuild (order 100), the fixture loader (200), the
 * price seeder (250), the dev-only demo-data loader (260), and the bootstrap admin (300).
 */
@Component
class StartupDataInitializer(
    private val startupTasks: List<StartupTaskService>
